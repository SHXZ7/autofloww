...
export const Cover = ({ children, className }) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className="relative group/cover inline-block bg-[#1e293b] hover:bg-[#1e3a5f] px-2 py-1 transition duration-200 rounded-md"
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
...
export const CircleIcon = ({ className, delay }) => {
  return (
    <div
      className={cn(
        "pointer-events-none animate-pulse group-hover/cover:opacity-100 h-2 w-2 rounded-full bg-[#3b82f6] opacity-20 group-hover/cover:bg-white",
        className
      )}
    />
  )
